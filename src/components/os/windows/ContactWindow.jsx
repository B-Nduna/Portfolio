...
export default function ContactWindow() {
  ...
  return (
    <div className="os-content">
      ...
      <form className="contact-form os-contact-form" onSubmit={onSubmit}>
        {FIELDS.map((f) => (
          <div className="field" key={f.id}>
            <input type={f.type} name={f.id} id={`os-${f.id}`} placeholder=" " required value={form[f.id]} onChange={onChange} />
            <label htmlFor={`os-${f.id}`}>{f.label}</label>
          </div>
        ))}
        <div className="field">
          <textarea name="message" id="os-message" rows={3} placeholder=" " required value={form.message} onChange={onChange} />
          <label htmlFor="os-message">Message</label>
        </div>
